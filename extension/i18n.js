...
(function (root) {
  const DICT = {
    en: {
      ...
      // 설정
      'ui.manual': '+ Manual',
      'read.none': 'No requests yet',
      'read.some': 'Requests recorded',
      'read.last': 'last {t}',
      'read.all_proxy': 'Every request came through a mail proxy, so the place and device below describe the proxy — not the reader.',
      'read.known_proxy': 'Every request whose origin was recorded came through a mail proxy, so the place and device below describe the proxy — not the reader.',
      'read.all_direct': 'Every request loaded straight from a mail client. Read the log below and judge for yourself.',
      'read.known_direct': 'Every request whose origin was recorded loaded straight from a mail client. Read the log below and judge for yourself.',
      'read.unknown': 'These records predate origin tracking, so there is no telling which of them came through a mail proxy.',
      'ev.none_recorded': 'No requests recorded yet.',
      'ev.none_filtered': 'Nothing was kept out. Every request the pixel received is in Recorded.',
      'ev.kept_out': 'This request was recorded but kept out of the log above.',
      'ev.full_record': 'Full record',
      'ev.origin_unknown': 'origin unknown',
      'meta.sent': 'sent {t}',
      'ev.since_sent': '{n}s after send',
      'ev.since_sent_m': '{n}m after send',
      'meta.ip_filtered': 'your own IP filtered',
      'meta.ip_not_recorded': 'sender IP not recorded',
      'meta.to': 'to {r}',
      'time.not_recorded': 'time not recorded',
    ...
    ko: {
      ...
      'read.some': '요청 기록됨',
      'read.last': '마지막 {t}',
      'read.all_proxy': '모든 요청이 메일 프록시를 거쳐 왔어요. 아래 장소와 기기는 읽은 사람이 아니라 프록시를 나타내요.',
      'read.known_proxy': '출처가 기록된 요청은 모두 메일 프록시를 거쳐 왔어요. 아래 장소와 기기는 읽은 사람이 아니라 프록시를 나타내요.',
      'read.all_direct': '모든 요청이 메일 앱에서 직접 왔어요. 아래 기록을 보고 직접 판단하세요.',
      'read.known_direct': '출처가 기록된 요청은 모두 메일 앱에서 직접 왔어요. 아래 기록을 보고 직접 판단하세요.',
      'read.unknown': '출처 기록 기능이 생기기 전의 기록이라, 어느 것이 프록시를 거쳤는지 알 수 없어요.',
      'ev.none_recorded': '아직 기록된 요청이 없어요.',
      'ev.none_filtered': '걸러낸 것이 없어요. 픽셀이 받은 요청은 모두 기록됨에 있어요.',
      'ev.kept_out': '이 요청은 기록됐지만 위 목록에서는 제외됐어요.',
      'ev.full_record': '전체 기록',
      'ev.origin_unknown': '출처 불명',
      'meta.sent': '{t} 발송',
      'ev.since_sent': '발송 {n}초 뒤',
      'ev.since_sent_m': '발송 {n}분 뒤',
      'meta.ip_filtered': '본인 IP 걸러냄',
      'meta.ip_not_recorded': '발신자 IP 미기록',
      'meta.to': '받는 사람 {r}',
      'time.not_recorded': '시각 미기록',
